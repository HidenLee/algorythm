...
for test_case in range(1,T+1):
    ...
    X = N
    while len(list(pizza_list[i][1] for i in range(M) if pizza_list[i][1] != 0)) > 1:
        if pizza_list[i][1] != 0:
            pizza_list[i][1] = pizza_list[i][1] // 2
        else:
            if X < M:
                pizza_list[i] = [pizza_list[X][0], pizza_list[X][1]//2]
                pizza_list[X] = [0,0]
                X = X + 1
            else:
                pass
        i = i + 1
        i = i % N
    rst = list(pizza_list[i][0] for i in range(M) if pizza_list[i][1] != 0)
    print(f'#{test_case} {rst[0]}')