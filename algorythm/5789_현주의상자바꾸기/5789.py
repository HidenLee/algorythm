...
for test_case in range(1,T+1): 
    N, Q = tuple(map(int,input().split())) # N - 상자의 갯수, Q - 칠하기 횟수
    boxes = [0]*N
    for roop in range(1,Q+1):
        L, R = tuple(map(int,input().split())) # L~R까지 반복회차만큼 그리기
        for idx in range(N):
            if L-1 <= idx and idx <= R-1: 
                boxes[idx] = roop
    print(f'#{test_case}',end=' ') # 출력양식을 맞추기위해 포맷팅 활용
    print(*boxes,sep=' ')           # 리스트의 요소만 출력하기위해 asterlisk사용  
